...
const APPROVAL_REQUESTS = [
  ...
  },
  {
    ...
    priority: 'high',
    steps: [
      { name: 'Legal', status: 'rejected', date: '2025-01-31' }
    ]
  }
];
        ...
        <div className="lg:col-span-1 glass-card rounded-2xl border border-white/10 flex flex-col overflow-hidden">
          <div className="p-4 border-b border-white/5 flex space-x-2 overflow-x-auto no-scrollbar">
            {['all', 'pending', 'approved', 'rejected'].map((tab) => (
              ...
              >
                {tab === 'all' ? (currentLanguage === 'en' ? 'All' : '全部') :
                 tab === 'pending' ? (currentLanguage === 'en' ? 'Pending' : '待审批') :
                 tab === 'approved' ? (currentLanguage === 'en' ? 'Approved' : '已通过') :
                 (currentLanguage === 'en' ? 'Rejected' : '已驳回')}
              </button>
            ))}
          </div>
            ...
            {APPROVAL_REQUESTS.filter(r => filter === 'all' || r.status === filter).map((req) => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold border ${getStatusColor(req.status)}`}>
                    {req.status}
                  </div>
                ...
                <div className="flex items-center justify-between text-xs text-gray-400">
                  ...
                  {req.amount !== '-' && <span className="text-green-400 font-mono">{req.amount}</span>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="lg:col-span-2 glass-card rounded-2xl border border-white/10 flex flex-col overflow-hidden relative">
          {selectedReq ? (
                ...
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(selectedReq.status)}`}>
                        {selectedReq.status}
                      </span>
                    </div>
                    <div className="flex items-center space-x-4 text-sm text-gray-400">
                      <span className="flex items-center"><FileText className="w-4 h-4 mr-1.5" /> {selectedReq.id}</span>
                      <span className="flex items-center"><User className="w-4 h-4 mr-1.5" /> {selectedReq.requester}</span>
                      <span className="flex items-center"><Clock className="w-4 h-4 mr-1.5" /> {selectedReq.date}</span>
                    </div>
                  </div>
                  {selectedReq.amount !== '-' && (
                    <div className="text-right">
                      <div className="text-sm text-gray-400 mb-1">{currentLanguage === 'en' ? 'Total Amount' : '总金额'}</div>
                      <div className="text-2xl font-bold text-green-400 font-mono">{selectedReq.amount}</div>
                    </div>
                  )}
                </div>
                   ...
                   <div className="relative z-10 flex justify-between">
                      <div className="flex flex-col items-center space-y-2">
                         ...
                         <div className="text-center">
                            <div className="text-xs font-bold text-white">{currentLanguage === 'en' ? 'Submitted' : '已提交'}</div>
                      ...
                      {selectedReq.steps.map((step, idx) => (
                        <div key={idx} className="flex flex-col items-center space-y-2">
                           <div className={`w-8 h-8 rounded-full flex items-center justify-center border-4 border-slate-900 shadow-lg transition-all ${
                             step.status === 'approved' ? 'bg-green-500 shadow-green-500/20' : 
                             step.status === 'rejected' ? 'bg-red-500 shadow-red-500/20' : 
                             step.status === 'current' ? 'bg-amber-500 animate-pulse shadow-amber-500/20' : 
                             'bg-slate-700'
                           }`}>
                           ...
                           <div className="text-center">
                              <div className={`text-xs font-bold ${step.status === 'pending' ? 'text-gray-500' : 'text-white'}`}>{step.name}</div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                 <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-1">
                       <label className="text-xs text-gray-500 uppercase">{currentLanguage === 'en' ? 'Department' : '部门'}</label>
                       <div className="text-white bg-slate-800/50 px-3 py-2 rounded-lg border border-white/5">{selectedReq.department}</div>
                    </div>
                    <div className="space-y-1">
                       <label className="text-xs text-gray-500 uppercase">{currentLanguage === 'en' ? 'Priority' : '优先级'}</label>
                       <div className="text-white bg-slate-800/50 px-3 py-2 rounded-lg border border-white/5 flex items-center">
                          {selectedReq.priority === 'high' && <AlertCircle className="w-3 h-3 text-red-400 mr-2" />}
                 ...
                 <div className="space-y-2">
                    <label className="text-xs text-gray-500 uppercase">{currentLanguage === 'en' ? 'Description' : '描述'}</label>
                 ...
                 <div className="space-y-2">
                    <label className="text-xs text-gray-500 uppercase">{currentLanguage === 'en' ? 'Attachments' : '附件'}</label>
                    <div className="grid grid-cols-2 gap-4">
                       <div className="flex items-center p-3 rounded-lg bg-slate-800/50 border border-white/5 hover:border-green-500/30 transition-colors cursor-pointer group">
                          ...
                          <div className="flex-1 min-w-0">
                             <div className="text-sm text-white truncate group-hover:text-green-400 transition-colors">Quotation.pdf</div>
                       ...
                       <div className="flex items-center p-3 rounded-lg bg-slate-800/50 border border-white/5 hover:border-green-500/30 transition-colors cursor-pointer group">
                          ...
                          <div className="flex-1 min-w-0">
                             <div className="text-sm text-white truncate group-hover:text-green-400 transition-colors">Specs.docx</div>
            ...
            <div className="flex flex-col items-center justify-center h-full text-gray-500">
               ...
               <p>{currentLanguage === 'en' ? 'Select a request to view details' : '选择一个申请以查看详情'}</p>
            </div>
          )}
        </div>
      </div>
