...
const WORKFLOW_INSTANCES: WorkflowInstance[] = [
  { id: 'INS-001', templateName: '废金属采购审批', initiator: '张伟', currentStep: '总经理审批', status: 'running', startTime: '2026-03-13 09:30', progress: 65, priority: 'high' },
  { id: 'INS-002', templateName: '费用报销审批', initiator: '李明', currentStep: '财务审核', status: 'running', startTime: '2026-03-13 10:15', progress: 50, priority: 'medium' },
  { id: 'INS-003', templateName: '物流发货审批', initiator: '王芳', currentStep: '已完成', status: 'completed', startTime: '2026-03-13 08:00', progress: 100, priority: 'high' },
  { id: 'INS-004', templateName: '合同会签审批', initiator: '赵强', currentStep: '法务审核', status: 'running', startTime: '2026-03-12 16:00', progress: 30, priority: 'medium' },
  { id: 'INS-005', templateName: '废金属采购审批', initiator: '刘洋', currentStep: '价格审核', status: 'paused', startTime: '2026-03-12 14:00', progress: 40, priority: 'low' },
  { id: 'INS-006', templateName: '新员工入职', initiator: 'HR部', currentStep: '已拒绝', status: 'rejected', startTime: '2026-03-11 09:00', progress: 25, priority: 'low' },
  { id: 'INS-007', templateName: '费用报销审批', initiator: '陈静', currentStep: '部门主管审批', status: 'timeout', startTime: '2026-03-10 11:00', progress: 25, priority: 'high' },
  { id: 'INS-008', templateName: '物流发货审批', initiator: '周鹏', currentStep: '运输安排', status: 'running', startTime: '2026-03-13 07:45', progress: 75, priority: 'medium' },
];
...
export function WorkflowDesigner({ currentLanguage }: WorkflowDesignerProps) {
  ...
  const stepTypeConf: Record<string, { label: string; color: string; bg: string }> = {
    ...
    condition: { label: isZh ? '条件' : 'Condition', color: 'text-amber-400', bg: 'bg-amber-500/20' },
  ...
  const statusConf: Record<string, { label: string; color: string; bg: string; icon: typeof CheckCircle2 }> = {
    running: { label: isZh ? '运行中' : 'Running', color: 'text-blue-400', bg: 'bg-blue-500/20', icon: Play },
    completed: { label: isZh ? '已完成' : 'Done', color: 'text-emerald-400', bg: 'bg-emerald-500/20', icon: CheckCircle2 },
    rejected: { label: isZh ? '已拒绝' : 'Rejected', color: 'text-red-400', bg: 'bg-red-500/20', icon: XCircle },
    paused: { label: isZh ? '已暂停' : 'Paused', color: 'text-amber-400', bg: 'bg-amber-500/20', icon: Pause },
    timeout: { label: isZh ? '已超时' : 'Timeout', color: 'text-orange-400', bg: 'bg-orange-500/20', icon: Clock },
  };
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">{isZh ? '可视化流程设计与智能审批引擎' : 'Visual workflow designer & smart approval engine'}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: isZh ? '活跃模板' : 'Active Templates', value: totalTemplates.toString(), icon: Layers, gradient: 'from-orange-500 to-red-500', sub: isZh ? '5类业务流程' : '5 categories' },
          { label: isZh ? '运行中实例' : 'Running', value: runningCount.toString(), icon: Play, gradient: 'from-blue-500 to-indigo-500', sub: isZh ? '需关注处理' : 'Need attention' },
          { label: isZh ? '本月完成' : 'Completed(M)', value: '102', icon: CheckCircle2, gradient: 'from-emerald-500 to-green-500', sub: '+16% MoM' },
          { label: isZh ? '平均审批时效' : 'Avg. Duration', value: isZh ? '15.2h' : '15.2h', icon: Clock, gradient: 'from-purple-500 to-pink-500', sub: '-22% MoM' },
        ].map((stat, idx) => {
          const StatIcon = stat.icon;
          return (
            <div key={idx} className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-4 hover:border-orange-500/20 transition-all">
      ...
      {activeTab === 'templates' && (
        <div className="space-y-4">
          {WORKFLOW_TEMPLATES.map(template => {
            ...
            return (
              <div key={template.id} onClick={() => setSelectedTemplate(isExpanded ? null : template.id)}
                className={`bg-slate-800/40 backdrop-blur-xl rounded-2xl border transition-all cursor-pointer ${
                  isExpanded ? 'border-orange-500/30 shadow-lg shadow-orange-500/5' : 'border-white/5 hover:border-white/10'
                }`}>
                <div className="p-5">
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        <div className="flex items-center space-x-2">
                          ...
                          <span className={`text-xs px-2 py-0.5 rounded-full bg-slate-700/50 ${cc.color}`}>{cc.label}</span>
                          <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400">{template.steps.length}{isZh ? '步' : ' steps'}</span>
                        </div>
                  ...
                  <div className="flex items-center space-x-6 text-xs text-gray-500">
                    <span>{isZh ? '运行次数' : 'Runs'}: <span className="text-white">{template.instanceCount}</span></span>
                    <span>{isZh ? '平均时长' : 'Avg.'}: <span className="text-white">{template.avgDuration}</span></span>
                    <span>{isZh ? '更新' : 'Updated'}: {template.updatedAt}</span>
                  </div>
                </div>
                ...
                {isExpanded && (
                  <div className="px-5 pb-5 border-t border-white/5 pt-4">
                    <p className="text-xs text-gray-500 mb-3">{isZh ? '流程步骤可视化' : 'Workflow Steps'}</p>
                    <div className="flex items-center flex-wrap gap-2">
                      {template.steps.map((step, si) => {
                        const sc = stepTypeConf[step.type];
                        return (
                          <React.Fragment key={step.id}>
                            <div className={`px-3 py-2 rounded-xl ${sc.bg} border border-white/5 min-w-[100px]`}>
                              <div className="flex items-center space-x-1.5 mb-1">
                                <div className={`w-2 h-2 rounded-full ${sc.color === 'text-emerald-400' ? 'bg-emerald-400' : sc.color === 'text-blue-400' ? 'bg-blue-400' : sc.color === 'text-amber-400' ? 'bg-amber-400' : sc.color === 'text-purple-400' ? 'bg-purple-400' : sc.color === 'text-cyan-400' ? 'bg-cyan-400' : 'bg-gray-400'}`} />
                                <span className={`text-xs ${sc.color}`}>{sc.label}</span>
                              </div>
                              <p className="text-sm text-white">{step.name}</p>
                              {step.assignee && <p className="text-xs text-gray-500 mt-0.5">{step.assignee}</p>}
                              {step.sla && <p className="text-xs text-amber-400/70 mt-0.5">SLA: {step.sla}</p>}
                            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/5">
                    <th className="text-left text-xs text-gray-500 py-3 px-4">{isZh ? '实例' : 'Instance'}</th>
                    <th className="text-left text-xs text-gray-500 py-3 px-4">{isZh ? '流程' : 'Workflow'}</th>
                    <th className="text-left text-xs text-gray-500 py-3 px-4">{isZh ? '发起人' : 'Initiator'}</th>
                    <th className="text-left text-xs text-gray-500 py-3 px-4">{isZh ? '当前节点' : 'Current Step'}</th>
                    ...
                    <th className="text-center text-xs text-gray-500 py-3 px-4">{isZh ? '优先级' : 'Priority'}</th>
                  </tr>
                </thead>
                <tbody>
                  {WORKFLOW_INSTANCES.map(inst => {
                    ...
                    const priorityConf = {
                      high: { label: isZh ? '高' : 'High', color: 'text-red-400', bg: 'bg-red-500/20' },
                      medium: { label: isZh ? '中' : 'Med', color: 'text-amber-400', bg: 'bg-amber-500/20' },
                      low: { label: isZh ? '低' : 'Low', color: 'text-gray-400', bg: 'bg-gray-500/20' },
                    };
                    ...
                    return (
                      <tr key={inst.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center space-x-2">
                            <div className="w-16 h-1.5 bg-slate-700/40 rounded-full overflow-hidden">
                              <div className={`h-full rounded-full ${inst.progress === 100 ? 'bg-emerald-500' : 'bg-blue-500'}`}
                        ...
                        <td className="py-3 px-4 text-center">
                          <span className={`inline-flex items-center space-x-1 text-xs px-2 py-0.5 rounded-full ${sc.bg} ${sc.color}`}>
                            <StatusIcon className="w-3 h-3" /><span>{sc.label}</span>
                          </span>
                        </td>
                        <td className="py-3 px-4 text-center">
                          <span className={`text-xs px-2 py-0.5 rounded-full ${pc.bg} ${pc.color}`}>{pc.label}</span>
                        </td>
                      </tr>
                    );
                  })}
          ...
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex space-x-2">
                <button className="px-3 py-1.5 bg-emerald-500/20 text-emerald-300 rounded-lg border border-emerald-500/30 text-xs">{isZh ? '保存草稿' : 'Save Draft'}</button>
                <button className="px-3 py-1.5 bg-orange-500/20 text-orange-300 rounded-lg border border-orange-500/30 text-xs">{isZh ? '发布' : 'Publish'}</button>
              </div>
            </div>

            {/* Node Palette */}
            <div className="mb-6">
              <p className="text-xs text-gray-500 mb-3">{isZh ? '拖拽节点到画布' : 'Drag nodes to canvas'}</p>
              <div className="flex flex-wrap gap-2">
                {Object.entries(stepTypeConf).map(([type, conf]) => (
                  <div key={type} className={`${conf.bg} border border-white/5 rounded-xl px-4 py-2.5 cursor-grab hover:border-white/15 transition-all`}>
            ...
            <div className="h-80 bg-slate-900/40 rounded-xl border border-dashed border-white/10 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-gray-500">{isZh ? '拖拽节点到此处开始设计流程' : 'Drag nodes here to start designing'}</p>
                <p className="text-xs text-gray-600 mt-1">{isZh ? '支持审批、条件分支、并行、子流程等' : 'Supports approval, conditions, parallel, sub-flows'}</p>
            ...
            <div className="mt-4 p-4 rounded-xl bg-gradient-to-r from-orange-500/10 to-red-500/5 border border-orange-500/15">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <p className="text-xs text-orange-400 mb-1">AI {isZh ? '设计建议' : 'Design Tips'}</p>
                  <ul className="text-xs text-gray-400 space-y-1">
                    <li>• {isZh ? '采购金额>50万建议增加总经理审批节点' : 'Add GM approval for procurement >¥500K'}</li>
                    <li>• {isZh ? '建议为关键审批节点设置SLA时限和超时升级机制' : 'Set SLA and escalation for critical approval nodes'}</li>
                    <li>• {isZh ? '可添加并行分支让法务和财务同时审核以缩短流程时间' : 'Add parallel branch for legal & finance to shorten process'}</li>
                  </ul>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <h3 className="text-white mb-4">{isZh ? '月度流程执行趋势' : 'Monthly Workflow Trend'}</h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                  <BarChart data={ANALYTICS_TREND}>
                    ...
                    <Bar dataKey="completed" fill="#22c55e" name={isZh ? '完成' : 'Completed'} radius={[0, 0, 0, 0]} />
                    <Bar dataKey="rejected" fill="#ef4444" name={isZh ? '拒绝' : 'Rejected'} radius={[0, 0, 0, 0]} />
                    <Bar dataKey="timeout" fill="#f59e0b" name={isZh ? '超时' : 'Timeout'} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { node: isZh ? '总经理审批' : 'GM Approval', avgWait: '6.8h', instances: 23, suggestion: isZh ? '建议对<20万的订单授权采购总监直接审批' : 'Delegate <¥200K to procurement director' },
                { node: isZh ? '法务审核' : 'Legal Review', avgWait: '18.2h', instances: 15, suggestion: isZh ? '建议增加法务助理分流标准合同审核' : 'Add legal assistant for standard contracts' },
                { node: isZh ? '财务审核' : 'Finance Check', avgWait: '5.5h', instances: 38, suggestion: isZh ? '建议AI预审自动通过低风险报销单' : 'AI pre-approve low-risk reimbursements' },
              ].map((item, idx) => (
                <div key={idx} className="p-4 rounded-xl bg-slate-900/40 border border-white/5">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-amber-400">{isZh ? '平均等待' : 'Avg.'}: {item.avgWait}</span>
                  </div>
                  <p className="text-xs text-gray-500 mb-2">{isZh ? '积压' : 'Pending'}: {item.instances} {isZh ? '个实例' : 'instances'}</p>
                  <div className="p-2 rounded-lg bg-orange-500/10 border border-orange-500/15">
                    <p className="text-xs text-orange-300"><Sparkles className="w-3 h-3 inline mr-1" />{item.suggestion}</p>
                  </div>
                </div>
              ))}
            </div>
