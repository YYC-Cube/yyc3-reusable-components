...
const AUTO_RULES: AutoRule[] = [
  {
    ...
    executionCount: 89,
    successRate: 96.6,
    lastTriggered: '2026-03-13 11:30',
    savedHours: 156,
  },
  {
    id: 'AR-002',
    ...
    executionCount: 34,
    successRate: 100,
    lastTriggered: '2026-03-13 09:45',
    savedHours: 68,
  },
  {
    id: 'AR-003',
    ...
    executionCount: 256,
    successRate: 94.1,
    lastTriggered: '2026-03-13 14:00',
    savedHours: 320,
  },
  {
    id: 'AR-004',
  ...
  },
  {
    ...
    executionCount: 18,
    successRate: 100,
    lastTriggered: '2026-03-11 10:00',
    savedHours: 36,
  },
  {
    id: 'AR-006',
    ...
    executionCount: 23,
    successRate: 91.3,
    lastTriggered: '2026-03-13 08:20',
    savedHours: 28,
  },
  {
    id: 'AR-007',
    ...
    executionCount: 5,
    successRate: 100,
    lastTriggered: '2026-03-01 00:05',
    savedHours: 40,
  },
];
...
const EXECUTIONS: Execution[] = [
  {
    ...
    trigger: '废铜6#库存<50吨',
    status: 'success',
    timestamp: '2026-03-13 11:30',
    duration: '2.3s',
    detail: '已生成采购单PO-20260313-042',
  },
  {
    id: 'E002',
    ruleName: '发票自动匹配',
    trigger: '新发票INV-88432',
    status: 'success',
    timestamp: '2026-03-13 14:00',
    duration: '0.8s',
    detail: '匹配订单ORD-20260310-018',
  },
  {
    id: 'E003',
    ruleName: '大额订单审核',
  ...
  },
  {
    id: 'E005',
    ruleName: '发票自动匹配',
    trigger: '新发票INV-88430',
    status: 'failed',
    timestamp: '2026-03-13 13:45',
    duration: '1.2s',
    detail: '金额差异5.3%超阈值，已转人工',
  },
  {
    id: 'E006',
    ruleName: '逾期催收',
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              {isZh
                ? 'RPA规则引擎·事件驱动·智能执行'
                : 'RPA rule engine · Event-driven · Smart execution'}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: isZh ? '活跃规则' : 'Active Rules',
            value: AUTO_RULES.filter((r) => r.status === 'active').length.toString(),
            icon: Zap,
            gradient: 'from-yellow-500 to-orange-500',
            sub: `${AUTO_RULES.length} ${isZh ? '总规则' : 'total'}`,
          },
          {
            label: isZh ? '今日执行' : 'Today Runs',
            value: '54',
            icon: Activity,
            gradient: 'from-blue-500 to-indigo-500',
            sub: isZh ? '1次失败' : '1 failed',
          },
          {
            label: isZh ? '累计节省工时' : 'Hours Saved',
            value: `${totalSavedHours}h`,
            icon: Clock,
            gradient: 'from-emerald-500 to-green-500',
            sub: `≈ ¥${Math.round((totalSavedHours * 80) / 10000)}万`,
          },
          {
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-lg transition-all ${activeTab === tab.id ? 'bg-gradient-to-r from-yellow-500/20 to-orange-500/20 text-white border border-yellow-500/30' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
            >
              <T className="w-4 h-4" />
              ...
              >
                <div className="p-5">
                  ...
                  <div className="flex items-center space-x-6 text-xs text-gray-500">
                    ...
                    <span>
                      {isZh ? '成功率' : 'Success'}:{' '}
                      <span className="text-emerald-400">{rule.successRate}%</span>
                    </span>
                    <span>
                      {isZh ? '最后触发' : 'Last'}: {rule.lastTriggered}
                    </span>
                  </div>
                </div>
                {isExpanded && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        {rule.conditions.map((c, i) => (
                          <div
                            key={i}
                            className="flex items-center space-x-2 text-xs text-amber-300 mb-1.5"
                          >
                      ...
                      <div>
                        ...
                        {rule.actions.map((a, i) => (
                          <div
                            key={i}
                            className="flex items-center space-x-2 text-xs text-emerald-300 mb-1.5"
                          >
                            <span className="w-5 h-5 rounded bg-emerald-500/20 flex items-center justify-center text-emerald-400">
                              {i + 1}
                            </span>
                            <span>{a}</span>
                          </div>
              ...
              <tbody>
                {EXECUTIONS.map((ex) => {
                  ...
                  return (
                    <tr
                      key={ex.id}
                      className={`border-b border-white/5 hover:bg-white/[0.02] ${ex.status === 'failed' ? 'bg-red-500/[0.03]' : ''}`}
                    >
                      ...
                      <td className="py-3 px-4 text-center">
                        <span
                          className={`inline-flex items-center space-x-1 text-xs px-2 py-0.5 rounded-full ${ec.bg} ${ec.color}`}
                        >
                          <EI className="w-3 h-3" />
                          <span>{ec.label}</span>
                        </span>
                      </td>
      ...
      {activeTab === 'templates' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            {
              name: isZh ? '采购全流程自动化' : 'Procurement Automation',
              desc: isZh
                ? '需求→询价→比价→下单→收货→付款'
                : 'Request→RFQ→Compare→Order→Receive→Pay',
              rules: 6,
              savedHours: '80h/月',
            },
            {
              name: isZh ? '销售线索自动跟进' : 'Lead Auto Follow-up',
              desc: isZh ? '线索评分→自动分配→跟进提醒→转化追踪' : 'Score→Assign→Remind→Track',
              rules: 4,
              savedHours: '45h/月',
            },
            {
              name: isZh ? '财务月末自动结算' : 'Month-end Settlement',
              desc: isZh ? '对账→核销→报表→审计→归档' : 'Reconcile→Settle→Report→Audit→Archive',
              rules: 5,
              savedHours: '60h/月',
            },
            {
              name: isZh ? '仓库智能调度' : 'Warehouse Smart Dispatch',
              desc: isZh ? '入库→上架→拣货→出库→盘点' : 'Inbound→Shelve→Pick→Outbound→Count',
              rules: 5,
              savedHours: '55h/月',
            },
            {
              name: isZh ? '客户服务自动化' : 'Customer Service Auto',
              desc: isZh ? '工单→分类→分配→处理→回访' : 'Ticket→Classify→Assign→Handle→Follow-up',
              rules: 4,
              savedHours: '35h/月',
            },
            {
              name: isZh ? '合规自动检查' : 'Compliance Auto Check',
              desc: isZh ? '数据采集→规则检查→报告生成→预警通知' : 'Collect→Check→Report→Alert',
              rules: 3,
              savedHours: '25h/月',
            },
          ].map((t, i) => (
            <div
              key={i}
              className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5 hover:border-yellow-500/20 transition-all"
            >
              ...
              <div className="flex items-center justify-between text-xs text-gray-500">
                <span>
                  {t.rules} {isZh ? '条规则' : 'rules'}
                </span>
                <span className="text-emerald-400">
                  {isZh ? '预计节省' : 'Est.'} {t.savedHours}
                </span>
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
            <h3 className="text-white flex items-center space-x-2 mb-4">
              ...
              <span>AI {isZh ? '优化建议' : 'Optimization Tips'}</span>
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                {
                  text: isZh
                    ? '「发票自动匹配」失败率偏高(5.9%)，建议将金额差异阈值从2%调整为3%，预计可减少65%的误报'
                    : 'Invoice matching fail rate 5.9%, adjust threshold from 2% to 3%',
                },
                {
                  text: isZh
                    ? '检测到每周三14:00-16:00执行密度最高，建议错峰调度以平滑系统负载'
                    : 'Peak execution Wed 14-16, suggest staggering for load balance',
                },
                {
                  text: isZh
                    ? '「逾期催收」规则可增加AI情感分析，根据客户历史行为自适应催收策略和话术'
                    : 'Add AI sentiment analysis to collection rule for adaptive strategy',
                },
              ].map((tip, i) => (
                <div
                  key={i}
                  className="p-4 rounded-xl bg-gradient-to-r from-yellow-500/10 to-orange-500/5 border border-yellow-500/15"
                >
