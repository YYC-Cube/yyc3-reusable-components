      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400">
            {currentLanguage === 'en'
              ? 'Real-time fleet tracking & shipment monitoring'
              : '实时车队追踪与货物监控'}
          </p>
        </div>
            ...
            <div className="glass-card p-4 rounded-xl pointer-events-auto backdrop-blur-md border border-white/10 bg-slate-900/60">
              <div className="flex items-center space-x-2 text-white mb-2">
                ...
                <span className="font-semibold">
                  {currentLanguage === 'en' ? 'Live Tracking' : '实时追踪'}
                </span>
              </div>
          ...
          <div className="glass-card rounded-2xl border border-white/10 overflow-hidden flex flex-col h-[400px]">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
              {SHIPMENTS.map((shipment) => (
                <div
                  key={shipment.id}
                  className="p-3 rounded-xl bg-slate-800/30 border border-white/5 hover:bg-slate-700/50 transition-colors group cursor-pointer"
                >
                  ...
                  <div className="flex justify-between items-center text-[10px] text-gray-400">
                    <span>
                      {shipment.origin[currentLanguage === 'en' ? 'en' : 'zh'].split(' ')[0]}
                    </span>
                    <ArrowRight className="w-3 h-3 text-gray-600" />
                    <span>
                      {shipment.dest[currentLanguage === 'en' ? 'en' : 'zh'].split(' ')[0]}
                    </span>
                  </div>
                </div>
              ))}
