...
const CALENDAR_DAYS: Array<{ day: number; type: 'empty' | 'normal' | 'weekend' | 'today' }> =
  (() => {
    // March 2026 calendar
    const days: Array<{ day: number; type: 'empty' | 'normal' | 'weekend' | 'today' }> = [];
    // March 1 is Sunday (index 0)
    for (let i = 0; i < 0; i++) days.push({ day: 0, type: 'empty' });
    for (let d = 1; d <= 31; d++) {
      const dow = (d - 1) % 7; // simplified
      const isWeekend = dow === 6 || dow === 0;
      let type: 'normal' | 'weekend' | 'today' = 'normal';
      if (d === 13) type = 'today';
...
export function Attendance({ currentLanguage }: AttendanceProps) {
  ...
  const presentToday = ATTENDANCE_RECORDS.filter(
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">
            {isZh ? '排班与出勤统计管理' : 'Scheduling & attendance statistics'}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: isZh ? '请假' : 'On Leave',
            value: onLeaveToday.toString(),
            icon: Plane,
            gradient: 'from-blue-500 to-indigo-500',
          },
          {
            label: isZh ? '月出勤率' : 'Monthly Rate',
            value: '94.8%',
            icon: TrendingUp,
            ...
            <div className="grid grid-cols-7 gap-2">
              {CALENDAR_DAYS.map((cell, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`h-16 rounded-xl p-2 text-center cursor-pointer transition-all ${
                      isToday
                  ...
                  >
                    <span
                      className={`text-sm ${isToday ? 'text-cyan-400' : isWeekend ? 'text-gray-600' : 'text-gray-300'}`}
                    >
                      {cell.day}
                    </span>
      ...
      {activeTab === 'records' && (
        <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 overflow-hidden">
          <div className="p-4 border-b border-white/5">
            <p className="text-sm text-gray-400">
              {isZh ? '2026-03-13 打卡记录' : '2026-03-13 Attendance Records'}
            </p>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-center">
                        <span
                          className={`text-sm ${record.status === 'late' ? 'text-amber-400' : 'text-white'}`}
                        >
                          {record.clockIn}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-center">
                        <span
                          className={`text-sm ${record.status === 'early' ? 'text-orange-400' : 'text-white'}`}
                        >
                          {record.clockOut}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-center">
                        <span
                          className={`text-sm ${record.hours > 10 ? 'text-purple-400' : 'text-white'}`}
                        >
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-gray-400 text-sm">
              {isZh
                ? `共 ${LEAVE_REQUESTS.length} 条请假记录`
                : `${LEAVE_REQUESTS.length} leave requests`}
            </p>
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {req.department} · {isZh ? '审批人：' : 'Approver: '}
                        {req.approver}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-lg text-white">
                      {req.days}
                      <span className="text-sm text-gray-500">{isZh ? '天' : 'd'}</span>
                    </p>
                  </div>
                </div>
      ...
      {activeTab === 'statistics' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <h3 className="text-white mb-4">
                {isZh ? '本周出勤率 & 加班人数' : 'Weekly Attendance & Overtime'}
              </h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                  <BarChart data={WEEKLY_DATA}>
                    ...
                    <Bar
                      ...
                      fill="#06b6d4"
                      radius={[4, 4, 0, 0]}
                      name={isZh ? '出勤率%' : 'Attendance %'}
                    />
                    <Bar
                      dataKey="overtime"
                      fill="#8b5cf6"
                      radius={[4, 4, 0, 0]}
                      name={isZh ? '加班人数' : 'Overtime'}
                    />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ...
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <h3 className="text-white mb-4">
                {isZh ? '月度出勤率趋势 (%)' : 'Monthly Attendance Rate (%)'}
              </h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                  <LineChart data={MONTHLY_TREND}>
                    ...
                    <Line
                      ...
                      stroke="#06b6d4"
                      strokeWidth={3}
                      dot={{ fill: '#06b6d4', r: 5, strokeWidth: 2, stroke: '#1e293b' }}
                      name={isZh ? '出勤率' : 'Rate %'}
                    />
                  </LineChart>
                </ResponsiveContainer>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <h3 className="text-white mb-4">
                {isZh ? '请假类型分布' : 'Leave Type Distribution'}
              </h3>
            ...
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <span className="text-gray-400 text-sm">
                  {isZh ? '月均加班时长' : 'Avg. Monthly OT'}
                </span>
              </div>
              <p className="text-3xl text-white">
                18.5<span className="text-lg text-gray-500">{isZh ? '小时/人' : 'h/person'}</span>
              </p>
