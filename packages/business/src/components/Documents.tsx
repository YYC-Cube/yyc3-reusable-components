...
const RECENT_FILES = [
  { id: 1, name: 'Q1_Financial_Report.pdf', type: 'pdf', size: '2.4 MB', date: '2 hrs ago', author: 'Sarah Chen' },
  { id: 2, name: 'Safety_Protocol_v2.docx', type: 'doc', size: '1.1 MB', date: '5 hrs ago', author: 'Ahmed Al-Fahad' },
  { id: 3, name: 'Site_Inspection_Photos.zip', type: 'zip', size: '15.6 MB', date: 'Yesterday', author: 'Mohammed Q.' },
  { id: 4, name: 'Supplier_Contract_Template.pdf', type: 'pdf', size: '845 KB', date: '2 days ago', author: 'Legal Dept' },
  { id: 5, name: 'Inventory_Audit_Log.xlsx', type: 'xls', size: '450 KB', date: '3 days ago', author: 'System' },
];
...
export function Documents({ currentLanguage }: DocumentsProps) {
  ...
  const getFileIcon = (type: string) => {
    switch (type) {
      ...
      case 'xls': return <FileText className="w-8 h-8 text-green-400" />;
      case 'zip': return <Folder className="w-8 h-8 text-amber-400" />;
      case 'img': return <ImageIcon className="w-8 h-8 text-purple-400" />;
  ...
  return (
    <div className="p-8 space-y-6 max-w-[1800px] mx-auto h-[calc(100vh-80px)] flex flex-col">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-400 mt-1">
              {currentLanguage === 'en' ? 'Secure storage for enterprise documents' : '企业文档的安全存储'}
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col space-y-8 overflow-y-auto pr-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
           <div className="md:col-span-3">
              <div className="relative">
                 ...
                 <input 
                    type="text" 
                    placeholder={currentLanguage === 'en' ? 'Search documents, folders, or content...' : '搜索文档、文件夹或内容...'}
        ...
        {/* Folders */}
        <div>
           <h3 className="text-lg font-bold text-white mb-4">{currentLanguage === 'en' ? 'Folders' : '文件夹'}</h3>
           <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {FOLDERS.map((folder) => (
                 <div key={folder.id} className="glass-card p-4 rounded-xl border border-white/10 hover:border-blue-500/30 transition-all cursor-pointer group hover:bg-slate-800/80">
                    <div className="flex justify-between items-start mb-3">
                       <div className={`p-2 rounded-lg ${folder.bg}`}>
                          <Folder className={`w-6 h-6 ${folder.color}`} />
                       </div>
                       <Button variant="ghost" size="icon" className="w-6 h-6 text-gray-500 hover:text-white">
                          <MoreVertical className="w-4 h-4" />
                       </Button>
                    </div>
                    <h4 className="text-white font-medium mb-1 group-hover:text-blue-400 transition-colors">{folder.name}</h4>
                    <p className="text-xs text-gray-500">{folder.count} {currentLanguage === 'en' ? 'files' : '个文件'}</p>
                 </div>
              ))}
              <div className="glass-card p-4 rounded-xl border border-dashed border-white/20 hover:border-blue-500/50 flex flex-col items-center justify-center cursor-pointer hover:bg-white/5 transition-all">
                 ...
                 <span className="text-xs text-gray-400">{currentLanguage === 'en' ? 'New Folder' : '新建文件夹'}</span>
              </div>
           </div>
        </div>
        ...
        <div className="flex-1">
           <h3 className="text-lg font-bold text-white mb-4">{currentLanguage === 'en' ? 'Recent Files' : '最近文件'}</h3>
           {viewMode === 'grid' ? (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                 {RECENT_FILES.map((file) => (
                    <div key={file.id} className="glass-card p-4 rounded-xl border border-white/10 hover:border-white/20 transition-all cursor-pointer group flex flex-col">
