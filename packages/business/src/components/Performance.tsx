    ...
    <div className="p-8 space-y-6 max-w-[1800px] mx-auto pb-20">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-400 mt-1">
              {currentLanguage === 'en' ? 'Track KPIs, goals, and employee growth' : '追踪 KPI、目标和员工成长'}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="glass-card p-5 rounded-2xl border border-white/10 relative overflow-hidden">
           ...
           <p className="text-slate-400 text-xs">{currentLanguage === 'en' ? 'Overall Productivity' : '整体生产力'}</p>
        </div>
        ...
        <div className="glass-card p-5 rounded-2xl border border-white/10 relative overflow-hidden">
           ...
           <p className="text-slate-400 text-xs">{currentLanguage === 'en' ? 'Dept Goals Met' : '部门目标达成'}</p>
        </div>
        ...
        <div className="glass-card p-5 rounded-2xl border border-white/10 relative overflow-hidden">
           ...
           <p className="text-slate-400 text-xs">{currentLanguage === 'en' ? 'Training Completion' : '培训完成率'}</p>
        </div>
        ...
        <div className="glass-card p-5 rounded-2xl border border-white/10 relative overflow-hidden">
           ...
           <p className="text-slate-400 text-xs">{currentLanguage === 'en' ? 'Safety Incidents' : '安全事故'}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
         ...
         <div className="lg:col-span-1 glass-card rounded-2xl p-6 border border-white/10 flex flex-col">
            <h3 className="text-lg font-bold text-white mb-6 flex items-center">
               <RadarChart className="w-5 h-5 mr-2 text-amber-400" /> // Icon placeholder, using text instead
            ...
            <div className="flex-1 min-h-[300px]">
               <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                  <RadarChart cx="50%" cy="50%" outerRadius="80%" data={PERFORMANCE_DATA}>
                     ...
                     <Radar name="Company Avg" dataKey="B" stroke="#f59e0b" strokeOpacity={0.6} fill="#f59e0b" fillOpacity={0.3} />
                     <Radar name="Dept Avg" dataKey="A" stroke="#3b82f6" strokeOpacity={0.6} fill="#3b82f6" fillOpacity={0.3} />
                     <Tooltip 
            ...
            <div className="flex justify-center space-x-4 mt-4 text-xs text-gray-400">
               <div className="flex items-center"><div className="w-3 h-3 bg-amber-500/50 mr-2 rounded-full"></div>Company Avg</div>
               <div className="flex items-center"><div className="w-3 h-3 bg-blue-500/50 mr-2 rounded-full"></div>Dept Avg</div>
            </div>
         </div>
         ...
         <div className="lg:col-span-2 glass-card rounded-2xl p-6 border border-white/10 flex flex-col">
            ...
            <div className="flex-1 min-h-[300px]">
               <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                  <BarChart data={DEPT_PERFORMANCE} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
      ...
      <div className="glass-card rounded-2xl p-6 border border-white/10">
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {TOP_PERFORMERS.map((person, idx) => (
               <div key={person.id} className="p-4 rounded-xl bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-white/5 flex items-center space-x-4 relative overflow-hidden group hover:border-amber-500/30 transition-colors">
                  {idx === 0 && <div className="absolute top-0 right-0 p-2"><Trophy className="w-12 h-12 text-amber-500/10 rotate-12" /></div>}
