...
export function useChatPersistence(channelId: string, initialChats: Chat[] = []) {
  ...
  const getStorageKey = (id: string) =>
    id === 'main' ? 'yyc3_chat_history' : `yyc3_chat_history_${id}`;
    ...
    (newChats: Chat[]) => {
      try {
        ...
        if (serialized.length > MAX_STORAGE_SIZE) {
          ...
          const cleaned = newChats.filter((c) => c.isStarred || new Date(c.updatedAt) > cutoff);
  ...
  const setChatsWrapper = useCallback(
    (value: Chat[] | ((val: Chat[]) => Chat[])) => {
      setChats((prev) => {
