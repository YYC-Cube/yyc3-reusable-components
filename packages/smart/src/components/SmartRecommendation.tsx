...
const SUPPLIER_RECS: SupplierRec[] = [
  {
    ...
    creditRating: 'AAA',
    region: '浙江',
    reason: 'Q1交货准时率98.5%，铜含量稳定>99.2%，历史合作满意度最高',
    savings: '¥12.8万/月',
  },
  {
    id: 'S002',
    ...
    creditRating: 'AA',
    region: '广东',
    reason: '华南地区物流成本最低，铝合金品类齐全，支持月结90天',
    savings: '¥8.5万/月',
  },
  {
    id: 'S003',
    ...
    creditRating: 'AA',
    region: '江苏',
    reason: '304/316不锈钢专供，AI质检合格率99.1%，可定制分拣',
    savings: '¥6.2万/月',
  },
  {
    id: 'S004',
    ...
    creditRating: 'A',
    region: '越南',
    reason: '进口铜价低于国内12%，LME挂钩定价，Q2扩产800吨/月',
    savings: '¥22.5万/月',
  },
  {
    id: 'S005',
    ...
    creditRating: 'AA',
    region: '山东',
    reason: '华北物流便捷，大宗废铁供应稳定，可承接1000吨/月',
    savings: '¥5.8万/月',
  },
];
...
const PRODUCT_RECS = [
  {
    ...
    confidence: 94,
    targetClients: 8,
    reason: 'Q2电子产业需求预测增长18%，库存可覆盖60天',
  },
  {
    id: 'P002',
    name: '6061铝合金废料',
  ...
  },
  {
    ...
    confidence: 82,
    targetClients: 3,
    reason: '短期需求下降，建议降低采购量，关注环保政策变化',
  },
];
...
const CUSTOMER_RECS = [
  ...
  },
  {
    ...
    crossSell: '-',
    churnRisk: 45,
    action: '流失预警！近30天无交互，建议安排客户经理回访',
  },
  {
    id: 'C004',
    name: '苏州联合金属',
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              {isZh
                ? 'AI驱动的供应商匹配·产品推荐·客户洞察'
                : 'AI-powered supplier matching · product rec · customer insight'}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: isZh ? '供应商匹配数' : 'Supplier Matches',
            value: '28',
            icon: Users,
            gradient: 'from-blue-500 to-indigo-500',
            sub: isZh ? '本月新增5个' : '5 new',
          },
          {
            label: isZh ? '采购节省' : 'Cost Saved',
            value: '¥55.8万',
            icon: TrendingUp,
            gradient: 'from-emerald-500 to-green-500',
            sub: isZh ? '本月累计' : 'This month',
          },
          {
            label: isZh ? '交叉销售转化' : 'Cross-sell',
            value: '18.5%',
            icon: ShoppingCart,
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-lg transition-all ${activeTab === tab.id ? 'bg-gradient-to-r from-pink-500/20 to-rose-500/20 text-white border border-pink-500/30' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
            >
              <T className="w-4 h-4" />
      ...
      {activeTab === 'supplier' && (
        <div className="space-y-4">
          {SUPPLIER_RECS.map((s) => (
            <div
              key={s.id}
              className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 hover:border-pink-500/20 transition-all p-5"
            >
              ...
              <div className="flex items-center space-x-6 text-xs text-gray-500">
                <span>
                  {isZh ? '价格' : 'Price'}: <span className="text-white">{s.priceIndex}</span>
                </span>
                <span>
                  {isZh ? '质量' : 'Quality'}: <span className="text-white">{s.qualityScore}</span>
                </span>
                <span>
                  {isZh ? '交期' : 'Delivery'}:{' '}
                  <span className="text-white">{s.deliveryScore}</span>
                </span>
                <span>
                  {isZh ? '信用' : 'Credit'}:{' '}
                  <span
                    className={`${s.creditRating === 'AAA' ? 'text-emerald-400' : s.creditRating === 'AA' ? 'text-blue-400' : 'text-amber-400'}`}
                  >
                    {s.creditRating}
                  </span>
                </span>
              ...
              <div className="flex items-start justify-between mb-2">
                <div>
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${p.demand === '高' ? 'bg-red-500/20 text-red-300' : p.demand === '中' ? 'bg-amber-500/20 text-amber-300' : 'bg-gray-500/20 text-gray-300'}`}
                    >
                      {isZh ? `需求${p.demand}` : `Demand: ${p.demand}`}
                    </span>
                ...
                <div className="text-right">
                  <p className="text-sm text-white">
                    AI {isZh ? '置信度' : 'Conf.'} {p.confidence}%
                  </p>
                  <p className="text-xs text-gray-500">
                    {p.targetClients} {isZh ? '个潜在客户' : 'clients'}
                  </p>
                </div>
              </div>
            </div>
      ...
      {activeTab === 'customer' && (
        <div className="space-y-4">
          {CUSTOMER_RECS.map((c) => (
            <div
              key={c.id}
              className={`bg-slate-800/40 backdrop-blur-xl rounded-2xl border p-5 ${c.churnRisk > 30 ? 'border-red-500/20' : 'border-white/5'}`}
            >
              <div className="flex items-start justify-between mb-2">
                <div>
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${c.potential === '高' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-amber-500/20 text-amber-300'}`}
                    >
                      {isZh ? `潜力${c.potential}` : c.potential}
                    </span>
                    {c.churnRisk > 30 && (
                      <span className="text-xs px-2 py-0.5 rounded-full bg-red-500/20 text-red-300 animate-pulse">
                        {isZh ? '流失预警' : 'Churn Risk'}
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-gray-500">
                    LTV: {c.ltv}{' '}
                    {c.crossSell !== '-' && `| ${isZh ? '交叉销售' : 'Cross-sell'}: ${c.crossSell}`}
                  </p>
                </div>
                <div className="flex items-center space-x-2">
                  <span className="text-xs text-gray-500">{isZh ? '流失风险' : 'Churn'}</span>
                  <span
                    className={`text-sm ${c.churnRisk > 30 ? 'text-red-400' : c.churnRisk > 15 ? 'text-amber-400' : 'text-emerald-400'}`}
                  >
                    {c.churnRisk}%
                  </span>
                </div>
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                <RadarChart data={MATCH_RADAR}>
                  ...
                  <Radar
                    name={isZh ? '华东铜业' : 'East China Copper'}
                    dataKey="A"
                    stroke="#ec4899"
                    fill="#ec4899"
                    fillOpacity={0.2}
