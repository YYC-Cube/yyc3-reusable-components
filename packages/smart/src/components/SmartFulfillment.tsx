...
const MOCK_ORDERS: FulfillmentOrder[] = [
  {
    id: 'FO-001',
    orderNo: 'ORD-2026-0318',
    customer: '山东齐鲁钢铁集团',
    product: { en: 'Steel HMS #1', zh: '废钢HMS1号' },
    quantity: '500吨',
    amount: 1425000,
    stage: 'in-transit',
    assignee: '张伟',
  ...
  {
    id: 'FO-002',
    orderNo: 'ORD-2026-0425',
    customer: 'Hyundai Metal Trading',
    product: { en: 'Copper Wire Scrap', zh: '废铜线' },
    quantity: '120吨',
    amount: 8160000,
    stage: 'quality-check',
    assignee: '李娜',
  ...
  {
    id: 'FO-003',
    orderNo: 'ORD-2026-0512',
    customer: 'PT Indo Copper Trading',
    product: { en: 'Bright Copper', zh: '光亮铜' },
    quantity: '80吨',
    amount: 5600000,
    stage: 'preparing',
    assignee: '王强',
  ...
  {
    id: 'FO-004',
    orderNo: 'ORD-2026-0198',
    customer: '江苏华鑫新材料',
    product: { en: 'Aluminum Alloy Scrap', zh: '铝合金废料' },
    quantity: '200吨',
    amount: 3640000,
    stage: 'delivered',
    assignee: '赵磊',
  ...
  {
    id: 'FO-005',
    orderNo: 'ORD-2026-0876',
    customer: '广东南海铝业集团',
    product: { en: 'Aluminum Ingot', zh: '铝锭' },
    quantity: '150吨',
    amount: 2775000,
    stage: 'completed',
    assignee: '陈芳',
  ...
  {
    id: 'FO-006',
    orderNo: 'ORD-2026-0633',
    customer: '福建三钢集团',
    product: { en: 'Steel Shredded', zh: '破碎料' },
    quantity: '300吨',
    amount: 855000,
    stage: 'confirmed',
    assignee: '刘洋',
...
export function SmartFulfillment({ currentLanguage }: SmartFulfillmentProps) {
  ...
  const activeOrders = MOCK_ORDERS.filter(
    (o) => !['completed', 'delivered'].includes(o.stage)
  ).length;
  const completedOrders = MOCK_ORDERS.filter((o) => o.stage === 'completed').length;
  const totalAmount = MOCK_ORDERS.filter((o) => o.stage !== 'completed').reduce(
    (s, o) => s + o.amount,
    0
  );
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              {isZh ? '订单履约全流程监控' : 'Full-cycle order fulfillment monitoring'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: isZh ? '在途金额' : 'In-transit Value',
            value: formatMoney(totalAmount),
            icon: DollarSign,
            gradient: 'from-amber-500 to-orange-500',
          },
          {
            label: isZh ? '准时率' : 'On-time Rate',
            value: '94%',
            icon: Clock,
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full ${stageConf.bg} ${stageConf.color}`}
                      >
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {isZh ? order.product.zh : order.product.en} · {order.quantity}
                      </p>
                    </div>
                    <div className="text-right">
                              ...
                              className={`w-8 h-8 rounded-full flex items-center justify-center ${
                                stage.status === 'done'
                                  ...
                                  : stage.status === 'current'
                                    ? 'bg-amber-500/30 border-amber-500/50 animate-pulse'
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <p className="text-sm text-white">
                            {isZh ? order.product.zh : order.product.en}
                          </p>
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <h3 className="text-white mb-4">
                {isZh ? '准时履约率趋势 (%)' : 'On-time Fulfillment Rate (%)'}
              </h3>
              <div className="h-64">
            ...
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <h3 className="text-white mb-4">
                {isZh ? '各环节平均耗时 (天)' : 'Avg. Stage Duration (Days)'}
              </h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <h3 className="text-white mb-4">
                {isZh ? '订单状态分布' : 'Order Status Distribution'}
              </h3>
            ...
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <span className="text-gray-400 text-sm">
                  {isZh ? '平均履约周期' : 'Avg. Fulfillment Cycle'}
                </span>
              </div>
              <p className="text-3xl text-white">10.8 {isZh ? '天' : 'days'}</p>
              <p className="text-xs text-emerald-400 mt-2">
                -1.2 {isZh ? '天较上月' : 'days vs last month'}
              </p>
              <div className="mt-4 pt-4 border-t border-white/5">
                <div className="flex items-center justify-between text-xs text-gray-500">
                  <span>
                    {isZh ? '最快' : 'Fastest'}: 5.2{isZh ? '天' : 'd'}
                  </span>
                  <span>
                    {isZh ? '最慢' : 'Slowest'}: 18.5{isZh ? '天' : 'd'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <span className="text-gray-400 text-sm">
                  {isZh ? '客户满意度' : 'Customer Satisfaction'}
                </span>
              </div>
              <p className="text-3xl text-white">
                4.7<span className="text-lg text-gray-500">/5</span>
              </p>
              <p className="text-xs text-emerald-400 mt-2">
                +0.2 {isZh ? '较上季度' : 'vs last quarter'}
              </p>
              <div className="mt-4 pt-4 border-t border-white/5 flex items-center space-x-1">
                {[1, 2, 3, 4, 5].map((s) => (
                  <div
                    key={s}
                    className={`w-6 h-6 rounded ${s <= 4 ? 'bg-amber-500/30' : 'bg-amber-500/10'} flex items-center justify-center`}
                  >
                    <span className={`text-xs ${s <= 4 ? 'text-amber-400' : 'text-amber-400/40'}`}>
                      ★
                    </span>
                  </div>
                ))}
