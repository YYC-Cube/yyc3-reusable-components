    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl text-white flex items-center space-x-2">
              <span>{isZh ? '自然语言处理' : 'Natural Language Processing'}</span>
              <Sparkles className="w-5 h-5 text-green-400 animate-pulse" />
            </h1>
            <p className="text-sm text-gray-400">
              {isZh
                ? '文本理解·实体提取·意图识别·情感分析'
                : 'Text understanding · NER · Intent · Sentiment'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: isZh ? '实体识别准确率' : 'NER Accuracy',
            value: '96.2%',
            icon: Target,
          ...
          {
            label: isZh ? '意图分类准确率' : 'Intent Accuracy',
            value: '94.8%',
            icon: Brain,
          ...
          {
            label: isZh ? '正面情感占比' : 'Positive Rate',
            value: '62%',
            icon: ThumbsUp,
            gradient: 'from-emerald-500 to-green-500',
            sub: isZh ? '行业平均55%' : 'Avg 55%',
          },
        ].map((s, i) => {
          const I = s.icon;
          return (
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-lg transition-all ${activeTab === tab.id ? 'bg-gradient-to-r from-green-500/20 to-emerald-500/20 text-white border border-green-500/30' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
            >
              <T className="w-4 h-4" />
        ...
        <div className="space-y-4">
          {ANALYSES.map((a) => {
            ...
            return (
              <div
                key={a.id}
                onClick={() => setExpandedItem(isExpanded ? null : a.id)}
                  ...
                  <div className="px-5 pb-5 border-t border-white/5 pt-4 space-y-3">
                    <div>
                      <p className="text-xs text-gray-500 mb-2">
                        {isZh ? '实体识别结果' : 'Named Entities'}
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {a.entities.map((e, i) => (
                          <span
                            key={i}
                            className={`text-xs px-3 py-1.5 rounded-lg bg-slate-900/40 border border-white/5 ${e.color}`}
                          >
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {a.keywords.map((k, i) => (
                          <span
                            key={i}
                            className="text-xs px-2 py-1 rounded-full bg-green-500/10 text-green-300 border border-green-500/20"
                          >
                            #{k}
                          </span>
                        ))}
      ...
      {activeTab === 'extraction' && (
        <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
          <h3 className="text-white mb-4">
            {isZh ? '实体提取统计（本月）' : 'Entity Extraction Stats (Month)'}
          </h3>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[
              {
                type: isZh ? '客户名称' : 'Customer',
                count: 856,
                accuracy: 98.2,
                color: 'text-blue-400',
              },
              {
                type: isZh ? '产品名称' : 'Product',
                count: 1243,
                accuracy: 96.8,
                color: 'text-amber-400',
              },
              {
                type: isZh ? '数量/规格' : 'Quantity',
                count: 980,
                accuracy: 95.5,
                color: 'text-emerald-400',
              },
              {
                type: isZh ? '价格/金额' : 'Price',
                count: 672,
                accuracy: 97.1,
                color: 'text-red-400',
              },
              {
                type: isZh ? '日期/时间' : 'Date',
                count: 534,
                accuracy: 99.0,
                color: 'text-purple-400',
              },
              {
                type: isZh ? '地点/地址' : 'Location',
                count: 312,
                accuracy: 94.3,
                color: 'text-pink-400',
              },
          ...
          <div className="mt-6 p-4 rounded-xl bg-green-500/10 border border-green-500/15">
            <p className="text-xs text-green-300">
              ...
              {isZh
                ? 'AI洞察：本月从客户邮件中自动提取采购意向126条，其中89条已自动创建线索并分配给销售团队，转化率达32%。建议优化"数量/规格"实体的提取规则，目前混合单位（吨/千克）的识别率偏低。'
                : 'AI extracted 126 purchase intents from emails, 89 auto-created as leads with 32% conversion. Suggest improving quantity/unit entity rules.'}
            </p>
          </div>
        </div>
      )}
              ...
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                <BarChart data={SENTIMENT_WEEKLY}>
                  ...
                  <Bar
                    dataKey="positive"
                    fill="#22c55e"
                    name={isZh ? '正面' : 'Positive'}
                    stackId="a"
                  />
                  <Bar
                    dataKey="neutral"
                    fill="#6b7280"
                    name={isZh ? '中性' : 'Neutral'}
                    stackId="a"
                  />
                  <Bar
                    dataKey="negative"
                    fill="#ef4444"
                    name={isZh ? '负面' : 'Negative'}
                    stackId="a"
                    radius={[4, 4, 0, 0]}
                  />
                </BarChart>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
            ...
            <div className="space-y-3">
              {[
                {
                  name: isZh ? '基础语言模型' : 'Base LLM',
                  value: 'YYC³-NLP-7B',
                  status: 'active',
                },
                {
                  name: isZh ? '行业微调版本' : 'Fine-tuned',
                  value: 'ScrapMetal-NLP-v3.2',
                  status: 'active',
                },
                {
                  name: isZh ? '支持语言' : 'Languages',
                  value: isZh ? '中文、英文、越南语' : 'CN, EN, VN',
                  status: 'active',
                },
                { name: isZh ? '推理延迟' : 'Latency', value: '28ms (P95)', status: 'active' },
                {
                  name: isZh ? '日处理容量' : 'Daily Cap',
                  value: '50,000 texts',
                  status: 'active',
                },
                {
                  name: isZh ? '模型更新频率' : 'Update Freq',
                  value: isZh ? '每周增量训练' : 'Weekly incremental',
                  status: 'active',
                },
              ].map((c, i) => (
                <div
