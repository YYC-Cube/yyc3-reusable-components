  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl text-white flex items-center space-x-2">
            <span>{isZh ? '智能文档' : 'Smart Documents'}</span>
            <Sparkles className="w-5 h-5 text-violet-400 animate-pulse" />
          </h1>
          <p className="text-sm text-gray-400">
            {isZh
              ? 'AI文档生成·版本管理·智能模板·协同编辑'
              : 'AI doc generation · Versioning · Templates · Collaboration'}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: isZh ? '文档总数' : 'Documents',
            value: '1,286',
            icon: FileText,
            gradient: 'from-violet-500 to-purple-500',
            sub: isZh ? '本月+48' : '+48 this month',
          },
          {
            label: isZh ? 'AI生成文档' : 'AI Generated',
            value: '126',
            icon: Brain,
            gradient: 'from-blue-500 to-indigo-500',
            sub: isZh ? '占比9.8%' : '9.8% of total',
          },
          {
            label: isZh ? '模板使用次数' : 'Template Uses',
            value: '1,324',
            icon: Layers,
            gradient: 'from-emerald-500 to-green-500',
            sub: isZh ? '本月' : 'This month',
          },
          {
            label: isZh ? '协作编辑' : 'Collaborating',
            value: '8',
            icon: Users,
            gradient: 'from-amber-500 to-orange-500',
            sub: isZh ? '人正在编辑' : 'editing now',
          },
        ].map((s, i) => {
          const I = s.icon;
          return (
        ...
        <div className="space-y-3">
          {DOCUMENTS.map((doc) => {
            ...
            return (
              <div
                key={doc.id}
                className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5 hover:border-violet-500/20 transition-all"
              >
                <div className="flex items-start justify-between">
                  <div>
      ...
      {activeTab === 'ai' && (
        <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-6">
          <h3 className="text-white flex items-center space-x-2 mb-4">
            ...
            <span>AI {isZh ? '文档助手' : 'Doc Assistant'}</span>
          </h3>
          <div className="h-48 bg-slate-900/40 rounded-xl border border-dashed border-white/10 flex items-center justify-center mb-4">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm">
                {isZh ? '描述需求，AI为您生成文档' : 'Describe your needs, AI generates docs'}
              </p>
              <button className="mt-3 px-4 py-2 bg-violet-500/20 text-violet-300 rounded-lg border border-violet-500/30 text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              isZh ? '合同草拟：输入交易条款自动生成合同' : 'Contract drafting from trade terms',
              isZh ? '报告生成：自动汇总数据生成分析报告' : 'Report generation from data summaries',
              isZh ? '标准文件：基于行业规范生成操作手册' : 'Standard docs from industry specs',
            ].map((t, i) => (
              <div key={i} className="p-3 rounded-xl bg-violet-500/10 border border-violet-500/15">
