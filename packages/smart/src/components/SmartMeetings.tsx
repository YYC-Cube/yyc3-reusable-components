...
const PAST_MEETINGS: Meeting[] = [
  {
    ...
    agenda: [],
    status: 'completed',
    aiSummary: 'Q1整体营收达成率108%，废铜业务同比增长22%，废铝增长15%。利润率较Q4提升1.8个百分点。重点关注：不锈钢库存周转率需改善，华南物流成本超预算8%。',
    actionItems: [
      { task: '制定不锈钢库存优化方案', assignee: '仓储部', deadline: '2026-03-20', status: 'pending' },
      { task: '华南物流成本专项审计', assignee: '财务部', deadline: '2026-03-25', status: 'pending' },
  ...
  },
  {
    ...
    agenda: [],
    status: 'completed',
    aiSummary: '与越南TPC达成初步合作意向：Q2供货量800吨/月，价格锁定机制采用LME月均价+加工费模式，首批交付预计4月中旬。需跟进合同细节和品质标准确认。',
    actionItems: [
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">{isZh ? 'AI驱动的智能会议管理与纪要生成' : 'AI-powered meeting management & minutes'}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: isZh ? '今日会议' : 'Today', value: todayMeetings.toString(), icon: Calendar, gradient: 'from-blue-500 to-indigo-500', sub: isZh ? '2个即将开始' : '2 starting soon' },
          { label: isZh ? '本周参会人次' : 'Participants(W)', value: totalParticipants.toString(), icon: Users, gradient: 'from-purple-500 to-pink-500', sub: isZh ? '跨5个部门' : 'Across 5 depts' },
          { label: isZh ? 'AI纪要生成' : 'AI Minutes', value: '48', icon: FileText, gradient: 'from-emerald-500 to-green-500', sub: isZh ? '本月自动生成' : 'Auto-generated' },
          { label: isZh ? '待办完成率' : 'Action Rate', value: '82%', icon: CheckCircle2, gradient: 'from-amber-500 to-orange-500', sub: '+12% MoM' },
        ].map((stat, idx) => {
          const StatIcon = stat.icon;
          return (
            <div key={idx} className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-4 hover:border-blue-500/20 transition-all">
      ...
      {activeTab === 'upcoming' && (
        <div className="space-y-4">
          {MEETINGS.map(meeting => {
            ...
            return (
              <div key={meeting.id} onClick={() => setExpandedMeeting(isExpanded ? null : meeting.id)}
                className={`bg-slate-800/40 backdrop-blur-xl rounded-2xl border transition-all cursor-pointer ${
                  isExpanded ? 'border-blue-500/30 shadow-lg shadow-blue-500/5' : 'border-white/5 hover:border-white/10'
                }`}>
                <div className="p-5">
                    ...
                    <div>
                      <div className="flex items-center space-x-2 mb-1">
                        ...
                        <span className="text-xs px-2 py-0.5 rounded-full bg-slate-700/50 text-gray-300">
                          <LocIcon className="w-3 h-3 inline mr-1" />{lc.label}
                        </span>
                        {meeting.type === 'urgent' && (
                          <span className="text-xs text-red-400 animate-pulse">● {isZh ? '紧急' : 'URGENT'}</span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center space-x-6 text-xs text-gray-500">
                    <span><Users className="w-3 h-3 inline mr-1" />{confirmedCount}/{meeting.participants.length} {isZh ? '已确认' : 'confirmed'}</span>
                    <span>{isZh ? '组织者' : 'Organizer'}: <span className="text-gray-300">{meeting.organizer}</span></span>
                    {meeting.room && <span><MapPin className="w-3 h-3 inline mr-1" />{meeting.room}</span>}
                  </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {meeting.aiFeatures.map((f, i) => (
                        <span key={i} className="text-xs px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-300 border border-blue-500/20">
                          <Brain className="w-3 h-3 inline mr-1" />{f}
                        </span>
                      ))}
                    </div>
                  )}
                ...
                {isExpanded && (
                  <div className="px-5 pb-5 border-t border-white/5 pt-4 space-y-4">
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {meeting.agenda.map((item, ai) => (
                          <div key={ai} className="flex items-start space-x-3 p-2 rounded-lg bg-slate-900/30">
                            <span className="w-5 h-5 rounded-full bg-blue-500/20 text-blue-400 text-xs flex items-center justify-center flex-shrink-0">{ai + 1}</span>
                    ...
                    {/* Participants */}
                    <div>
                      <p className="text-xs text-gray-500 mb-2">{isZh ? '参会人员' : 'Participants'}</p>
                      <div className="flex flex-wrap gap-2">
                        {meeting.participants.map((p, pi) => (
                          <div key={pi} className={`px-3 py-1.5 rounded-lg text-xs border ${
                            p.confirmed ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-300' : 'bg-slate-800/50 border-white/5 text-gray-400'
                          }`}>
                            {p.name} <span className="text-gray-500">({p.role})</span>
                            {p.confirmed ? <CheckCircle2 className="w-3 h-3 inline ml-1" /> : <Clock className="w-3 h-3 inline ml-1" />}
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="flex space-x-2">
                      ...
                      <button className="flex items-center space-x-1.5 px-4 py-2 bg-slate-700/30 text-gray-300 rounded-lg border border-white/10 hover:bg-slate-700/50 transition-all text-sm"
                        onClick={e => e.stopPropagation()}>
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="flex items-center space-x-2 mb-1">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${tc.bg} ${tc.color}`}>{tc.label}</span>
                        <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400">{isZh ? '已完成' : 'Done'}</span>
                      </div>
                      <h4 className="text-white">{meeting.title}</h4>
                    </div>
                    <span className="text-xs text-gray-500">{meeting.date} · {meeting.time}</span>
                  </div>
                  {meeting.aiSummary && (
                  ...
                  <div className="px-5 pb-5 border-t border-white/5 pt-4 space-y-4">
                    <div className="p-4 rounded-xl bg-gradient-to-r from-blue-500/10 to-indigo-500/5 border border-blue-500/15">
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          <p className="text-xs text-blue-400 mb-1">AI {isZh ? '会议摘要' : 'Meeting Summary'}</p>
                          <p className="text-sm text-gray-300 leading-relaxed">{meeting.aiSummary}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    {meeting.actionItems && meeting.actionItems.length > 0 && (
                      <div>
                        <p className="text-xs text-gray-500 mb-2">AI {isZh ? '提取待办' : 'Extracted Actions'}</p>
                        <div className="space-y-2">
                          {meeting.actionItems.map((ai, idx) => (
                            <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-slate-900/40 border border-white/5">
                              <div className="flex items-center space-x-3">
                                {ai.status === 'done' ? <CheckCircle2 className="w-4 h-4 text-emerald-400" /> : <Clock className="w-4 h-4 text-amber-400" />}
                                <div>
                                  <p className="text-sm text-white">{ai.task}</p>
                                  <p className="text-xs text-gray-500">{ai.assignee} · {isZh ? '截止' : 'Due'}: {ai.deadline}</p>
                                </div>
                              </div>
                              <span className={`text-xs px-2 py-0.5 rounded-full ${
                                ai.status === 'done' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'
                              }`}>
                                {ai.status === 'done' ? (isZh ? '已完成' : 'Done') : (isZh ? '待处理' : 'Pending')}
                              </span>
                            </div>
                          ))}
                        </div>
        ...
        <div className="space-y-6">
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-gray-500">{isZh ? '支持语音识别、意图提取、待办生成' : 'Speech recognition, intent extraction, action generation'}</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              {[
                { label: isZh ? '语音转文字准确率' : 'Speech-to-Text Acc.', value: '96.8%', icon: Mic, color: 'text-blue-400' },
                { label: isZh ? '关键议题提取率' : 'Topic Extraction', value: '93.2%', icon: Target, color: 'text-purple-400' },
                { label: isZh ? '待办自动提取率' : 'Action Extraction', value: '89.5%', icon: Zap, color: 'text-amber-400' },
              ].map((item, idx) => {
            ...
            <div className="p-5 rounded-xl bg-slate-900/30 border border-white/5">
              ...
              <div className="space-y-3 text-sm text-gray-300">
                <div>
                  <p className="text-xs text-blue-400 mb-1">{isZh ? '核心结论' : 'Key Conclusions'}</p>
                ...
                <div>
                  <p className="text-xs text-amber-400 mb-1">{isZh ? '待办事项（AI自动提取）' : 'Action Items (AI extracted)'}</p>
                  <ul className="list-disc list-inside text-xs text-gray-400 space-y-1">
        ...
        <div className="space-y-6">
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                  <BarChart data={MEETING_STATS_WEEKLY}>
                    ...
                    <Bar dataKey="meetings" fill="#3b82f6" name={isZh ? '会议数' : 'Meetings'} radius={[4, 4, 0, 0]} />
                    <Bar dataKey="actionItems" fill="#22c55e" name={isZh ? '待办数' : 'Actions'} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              ...
              <div className="h-64 flex items-center">
                <div className="w-1/2 h-full">
                  <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                    <PieChart>
                      <Pie data={TYPE_PIE} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={3} dataKey="value">
                        {TYPE_PIE.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                      </Pie>
                      <Tooltip contentStyle={{ background: 'rgba(15,23,42,0.95)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: '#fff' }} />
                    </PieChart>
                  </ResponsiveContainer>
          ...
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {[
                { label: isZh ? '人均会议时长/周' : 'Avg. Hours/Week', value: isZh ? '4.2小时' : '4.2h', change: '-18%', good: true },
                { label: isZh ? '会议准时开始率' : 'On-time Start', value: '87%', change: '+5%', good: true },
                { label: isZh ? '超时会议占比' : 'Overtime Rate', value: '15%', change: '-8%', good: true },
                { label: isZh ? '无议程会议占比' : 'No Agenda Rate', value: '8%', change: '-12%', good: true },
              ].map((item, idx) => (
                <div key={idx} className="p-4 rounded-xl bg-slate-900/40 border border-white/5">
                  <p className="text-xs text-gray-500 mb-2">{item.label}</p>
                  <p className="text-xl text-white">{item.value}</p>
                  <p className={`text-xs mt-1 ${item.good ? 'text-emerald-400' : 'text-red-400'}`}>{item.change} MoM</p>
                </div>
              ))}
            </div>
          </div>
