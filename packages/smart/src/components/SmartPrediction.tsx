...
const MARKET_INDICES = [
  ...
  { name: '废钢综合指数', value: '2,820', change: '-0.8%', trend: 'down' as const, color: '#6b7280' },
  { name: '人民币/美元', value: '7.18', change: '+0.15%', trend: 'up' as const, color: '#ef4444' },
  { name: '波罗的海干散货', value: '1,652', change: '-3.2%', trend: 'down' as const, color: '#8b5cf6' },
...
const AI_MODELS: PredictionModel[] = [
  { name: '铜价LSTM时序模型', accuracy: 94.2, lastUpdated: '2h前', features: 128, status: 'active' },
  { name: '铝价Transformer预测', accuracy: 91.5, lastUpdated: '3h前', features: 96, status: 'active' },
  { name: '废钢需求预测引擎', accuracy: 88.7, lastUpdated: '1h前', features: 64, status: 'active' },
  { name: '现金流XGBoost集成', accuracy: 92.1, lastUpdated: '30min前', features: 48, status: 'training' },
...
export function SmartPrediction({ currentLanguage }: SmartPredictionProps) {
  ...
  const metalConf = {
    copper: { label: isZh ? '废铜' : 'Copper Scrap', data: COPPER_PRICE_DATA, color: '#f97316', unit: '¥/吨', currentPrice: '¥72,800', predictQ2: '¥76,500', change: '+5.1%' },
    aluminum: { label: isZh ? '废铝' : 'Aluminum Scrap', data: ALUMINUM_PRICE_DATA, color: '#3b82f6', unit: '¥/吨', currentPrice: '¥20,200', predictQ2: '¥21,000', change: '+4.0%' },
    steel: { label: isZh ? '废钢' : 'Steel Scrap', data: STEEL_PRICE_DATA, color: '#6b7280', unit: '¥/吨', currentPrice: '¥2,820', predictQ2: '¥2,680', change: '-5.0%' },
  };
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <div className="relative">
            ...
            <div className={`absolute -inset-1.5 rounded-2xl border-2 border-blue-400/30 transition-all duration-1000 ${
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">{isZh ? 'AI多模型集成预测引擎' : 'AI multi-model ensemble prediction engine'}</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <div className="flex items-center space-x-2 px-3 py-1.5 bg-slate-800/60 rounded-lg border border-white/5">
            ...
            <span className="text-xs text-gray-400">{isZh ? '模型更新中...' : 'Models updating...'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-gray-500">{AI_MODELS.filter(m => m.status === 'active').length}/{AI_MODELS.length} {isZh ? '在线' : 'Online'}</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          {AI_MODELS.map((model, idx) => (
            <div key={idx} className="p-3 rounded-xl bg-slate-900/50 border border-white/5 hover:border-blue-500/20 transition-all">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-1.5">
                  <div className={`w-1.5 h-1.5 rounded-full ${model.status === 'active' ? 'bg-emerald-400' : model.status === 'training' ? 'bg-amber-400 animate-pulse' : 'bg-gray-500'}`} />
                  <span className="text-xs text-gray-300 truncate">{model.name}</span>
                </div>
              </div>
              <div className="flex items-end justify-between">
                <span className={`text-lg ${model.accuracy >= 90 ? 'text-emerald-400' : 'text-amber-400'}`}>{model.accuracy}%</span>
              ...
              <div className="h-1 bg-slate-700/40 rounded-full mt-2 overflow-hidden">
                <div className={`h-full rounded-full ${model.accuracy >= 90 ? 'bg-emerald-500' : 'bg-amber-500'}`}
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center space-x-3">
            {(Object.keys(metalConf) as Array<'copper' | 'aluminum' | 'steel'>).map(key => {
              ...
              return (
                <button key={key} onClick={() => setSelectedMetal(key)}
                  ...
                  <div className={`text-xs mt-0.5 ${m.change.startsWith('+') ? 'text-emerald-400' : 'text-red-400'}`}>
          ...
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white">{mc.label}{isZh ? '价格预测曲线' : ' Price Forecast'} ({mc.unit})</h3>
              <div className="flex items-center space-x-4 text-xs">
                <span className="flex items-center space-x-1.5"><div className="w-3 h-0.5 bg-white" /><span className="text-gray-400">{isZh ? '实际价格' : 'Actual'}</span></span>
                <span className="flex items-center space-x-1.5"><div className="w-3 h-0.5 rounded-full" style={{ backgroundColor: mc.color, opacity: 0.7 }} /><span className="text-gray-400">{isZh ? 'AI预测' : 'AI Predicted'}</span></span>
              </div>
            </div>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                <ComposedChart data={mc.data}>
                  ...
                  <Area type="monotone" dataKey="predicted" stroke={mc.color} fill={`url(#predGrad-${selectedMetal})`} strokeWidth={2} strokeDasharray="6 3" name={isZh ? 'AI预测' : 'Predicted'} connectNulls={false} />
                  <Line type="monotone" dataKey="actual" stroke="#ffffff" strokeWidth={2.5} dot={{ fill: '#fff', r: 4, strokeWidth: 2, stroke: '#1e293b' }} name={isZh ? '实际价格' : 'Actual'} connectNulls={false} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-white/5 grid grid-cols-3 gap-4 text-center">
              <div>
                ...
                <p className="text-lg text-emerald-400">{selectedMetal === 'copper' ? '94.2%' : selectedMetal === 'aluminum' ? '91.5%' : '88.7%'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">{isZh ? '预测偏差(MAPE)' : 'MAPE'}</p>
                <p className="text-lg text-white">{selectedMetal === 'copper' ? '2.1%' : selectedMetal === 'aluminum' ? '2.8%' : '3.5%'}</p>
              </div>
              <div>
      ...
      {activeTab === 'demand' && (
        <div className="space-y-6">
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
            <h3 className="text-white mb-4">{isZh ? '月度需求预测（吨）' : 'Monthly Demand Forecast (tons)'}</h3>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                <BarChart data={DEMAND_FORECAST}>
                  ...
                  <Bar dataKey="copper" stackId="a" fill="#f97316" name={isZh ? '废铜' : 'Copper'} radius={[0, 0, 0, 0]} />
                  <Bar dataKey="aluminum" stackId="a" fill="#3b82f6" name={isZh ? '废铝' : 'Aluminum'} radius={[0, 0, 0, 0]} />
                  <Bar dataKey="steel" stackId="a" fill="#6b7280" name={isZh ? '废钢' : 'Steel'} radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { label: isZh ? 'Q2废铜需求预测' : 'Q2 Copper Demand', value: isZh ? '4,300吨' : '4,300t', change: '+12%', icon: TrendingUp, color: 'text-orange-400' },
              { label: isZh ? 'Q2废铝需求预测' : 'Q2 Aluminum Demand', value: isZh ? '3,130吨' : '3,130t', change: '+8%', icon: TrendingUp, color: 'text-blue-400' },
              { label: isZh ? 'Q2废钢需求预测' : 'Q2 Steel Demand', value: isZh ? '7,250吨' : '7,250t', change: '-3%', icon: TrendingDown, color: 'text-gray-400' },
            ].map((item, idx) => {
              ...
              return (
                <div key={idx} className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
                  ...
                  <p className={`text-xs mt-1 ${item.change.startsWith('+') ? 'text-emerald-400' : 'text-red-400'}`}>{item.change} {isZh ? '环比' : 'QoQ'}</p>
                </div>
              );
            })}
          </div>
      ...
      {activeTab === 'cashflow' && (
        <div className="space-y-6">
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
            <h3 className="text-white mb-4">{isZh ? '现金流预测（万元）' : 'Cash Flow Forecast (10K ¥)'}</h3>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                <ComposedChart data={CASHFLOW_DATA}>
                  ...
                  <Tooltip contentStyle={{ background: 'rgba(15,23,42,0.95)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: '#fff' }} />
                  <Legend wrapperStyle={{ color: '#94a3b8', fontSize: 12 }} />
                  <Bar dataKey="inflow" fill="#22c55e" opacity={0.7} radius={[4, 4, 0, 0]} name={isZh ? '现金流入' : 'Inflow'} />
                  <Bar dataKey="outflow" fill="#ef4444" opacity={0.5} radius={[4, 4, 0, 0]} name={isZh ? '现金流出' : 'Outflow'} />
                  <Line type="monotone" dataKey="net" stroke="#f59e0b" strokeWidth={2.5} dot={{ fill: '#f59e0b', r: 4 }} name={isZh ? '净现金流' : 'Net'} />
                  <Line type="monotone" dataKey="predicted_net" stroke="#8b5cf6" strokeWidth={2} strokeDasharray="5 3" dot={{ fill: '#8b5cf6', r: 3 }} name={isZh ? 'AI预测净流' : 'Predicted Net'} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
              { label: isZh ? 'Q2预测净流入' : 'Q2 Net Inflow', value: '¥2,000万', icon: ArrowUpRight, color: 'text-emerald-400', gradient: 'from-emerald-500 to-green-500' },
              { label: isZh ? '资金缺口预警' : 'Gap Alert', value: isZh ? '无' : 'None', icon: CheckCircle2, color: 'text-emerald-400', gradient: 'from-blue-500 to-cyan-500' },
              { label: isZh ? '回款周期预测' : 'Collection Cycle', value: isZh ? '32天' : '32d', icon: Clock, color: 'text-amber-400', gradient: 'from-amber-500 to-orange-500' },
              { label: isZh ? '预测准确率' : 'Forecast Accuracy', value: '92.1%', icon: Target, color: 'text-purple-400', gradient: 'from-purple-500 to-pink-500' },
            ].map((item, idx) => {
              const Icon = item.icon;
              return (
                <div key={idx} className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-4">
                  <div className="flex items-center space-x-2 mb-2">
                    <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${item.gradient} opacity-80 flex items-center justify-center`}>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {MARKET_INDICES.map((idx, i) => (
              <div key={i} className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-4 hover:border-white/10 transition-all">
                <p className="text-xs text-gray-500 mb-1">{idx.name}</p>
                <p className="text-lg text-white">{idx.value}</p>
                <div className="flex items-center space-x-1 mt-1">
                  {idx.trend === 'up' ? <ArrowUpRight className="w-3 h-3 text-emerald-400" /> : <ArrowDownRight className="w-3 h-3 text-red-400" />}
                  <span className={`text-xs ${idx.trend === 'up' ? 'text-emerald-400' : 'text-red-400'}`}>{idx.change}</span>
                </div>
              </div>
            ))}
          </div>
                ...
                <div key={idx} className="p-4 rounded-xl bg-slate-900/40 border border-white/5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-full" style={{
                      backgroundColor: item.score >= 60 ? 'rgba(34,197,94,0.2)' : item.score <= 40 ? 'rgba(239,68,68,0.2)' : 'rgba(245,158,11,0.2)',
                      color: item.score >= 60 ? '#22c55e' : item.score <= 40 ? '#ef4444' : '#f59e0b'
                    }}>
                      {item.sentiment}
                    </span>
                  </div>
                  ...
                  <div className="mb-3">
                    ...
                    <div className="h-2 bg-gradient-to-r from-red-500/30 via-amber-500/30 to-emerald-500/30 rounded-full relative">
                      <div className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full border-2 border-white bg-slate-900 transition-all"
                        style={{ left: `${item.score}%`, transform: `translateX(-50%) translateY(-50%)` }} />
                    </div>
                    <p className="text-center text-xs mt-1" style={{ color: item.color }}>{item.score}/100</p>
                  </div>
