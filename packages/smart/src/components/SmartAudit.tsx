...
const FINDINGS: AuditFinding[] = [
  {
    ...
    severity: 'critical',
    category: '流程违规',
    description: '3笔50万以上采购未经总经理审批直接执行',
    recommendation: '立即补签审批，修订采购审批阈值制度',
    status: 'open',
    dueDate: '2026-03-15',
    owner: '采购总监',
  },
  {
    ...
    severity: 'major',
    category: '供应商管理',
    description: '新供应商准入评审记录不完整（缺少现场考察报告）',
    recommendation: '补充现场考察，完善准入流程模板',
    status: 'remediation',
    dueDate: '2026-03-20',
    owner: '供应链VP',
  },
  {
    ...
    severity: 'major',
    category: '信用管理',
    description: '2家客户超信用额度仍继续发货，金额共计¥85万',
    recommendation: '立即启动信用冻结，完善系统硬控措施',
    status: 'open',
    dueDate: '2026-03-18',
    owner: '风控经理',
  },
  {
    ...
    severity: 'minor',
    category: '盘点差异',
    description: '废铜库存账实差异0.8%（约3.2吨），在可接受范围',
    recommendation: '调整账面数据，优化称重流程减少误差',
    status: 'closed',
    dueDate: '2026-03-10',
    owner: '仓储经理',
  },
  {
    ...
    severity: 'major',
    category: '排放合规',
    description: '熔炼车间粉尘排放监测数据有2次超标记录',
    recommendation: '升级除尘设备，增加监测频率',
    status: 'remediation',
    dueDate: '2026-03-25',
    owner: '环保主管',
  },
  {
    ...
    category: '文档管理',
    description: '危废转移联单归档不及时，建议数字化管理',
    recommendation: '引入电子联单系统',
    status: 'accepted',
    dueDate: '2026-04-30',
    owner: 'IT部',
  },
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              {isZh
                ? 'AI风险识别·合规检查·审计追踪'
                : 'AI risk detection · Compliance · Audit tracking'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: isZh ? '进行中审计' : 'Active Audits',
            value: AUDIT_TASKS.filter((t) => t.status === 'in-progress').length.toString(),
            icon: FileText,
            gradient: 'from-indigo-500 to-blue-500',
            sub: `${AUDIT_TASKS.length} ${isZh ? '总任务' : 'total'}`,
          },
          {
            label: isZh ? '待整改发现' : 'Open Findings',
            value: openFindings.toString(),
            icon: AlertTriangle,
            gradient: 'from-amber-500 to-orange-500',
            sub: `${criticalFindings} ${isZh ? '严重' : 'critical'}`,
          },
          {
            label: isZh ? '整改完成率' : 'Fix Rate',
            value: `${Math.round((FINDINGS.filter((f) => f.status === 'closed').length / FINDINGS.length) * 100)}%`,
            icon: CheckCircle2,
            gradient: 'from-emerald-500 to-green-500',
            sub: '+8% MoM',
          },
          {
            label: isZh ? '合规评分' : 'Compliance Score',
            value: '82',
            icon: Shield,
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-lg transition-all ${activeTab === tab.id ? 'bg-gradient-to-r from-indigo-500/20 to-blue-500/20 text-white border border-indigo-500/30' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
            >
              <T className="w-4 h-4" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
            <h3 className="text-white mb-4">
              {isZh ? '月度审计发现趋势' : 'Monthly Findings Trend'}
            </h3>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                <BarChart data={MONTHLY_FINDINGS}>
                  ...
                  <Bar
                    dataKey="remediation"
                    fill="#f59e0b"
                    name={isZh ? '整改中' : 'Fixing'}
                    stackId="a"
                  />
                  <Bar
                    dataKey="closed"
                    fill="#22c55e"
                    name={isZh ? '已关闭' : 'Closed'}
                    stackId="a"
                    radius={[4, 4, 0, 0]}
                  />
                </BarChart>
        ...
        <div className="space-y-4">
          {AUDIT_TASKS.map((task) => {
            ...
            return (
              <div
                key={task.id}
                className={`bg-slate-800/40 backdrop-blur-xl rounded-2xl border p-5 ${task.riskLevel === 'high' ? 'border-red-500/10' : 'border-white/5'}`}
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="flex items-center space-x-2 mb-1">
                      ...
                      <span
                        className={`text-xs ${task.riskLevel === 'high' ? 'text-red-400' : task.riskLevel === 'medium' ? 'text-amber-400' : 'text-gray-400'}`}
                      >
                        {isZh
                          ? `风险${task.riskLevel === 'high' ? '高' : task.riskLevel === 'medium' ? '中' : '低'}`
                          : task.riskLevel}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500">
                      {isZh ? '范围' : 'Scope'}: {task.scope} | {isZh ? '审计组' : 'Auditor'}:{' '}
                      {task.auditor}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-white">
                      {task.findingsCount} {isZh ? '发现' : 'findings'}
                    </p>
                    <p className="text-xs text-gray-500">
        ...
        <div className="space-y-4">
          {FINDINGS.map((f) => {
            ...
            return (
              <div
                key={f.id}
                className={`bg-slate-800/40 backdrop-blur-xl rounded-2xl border p-5 ${f.severity === 'critical' ? 'border-red-500/20' : 'border-white/5'}`}
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="flex items-center space-x-2 mb-1">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${sv.bg} ${sv.color}`}>
                        {sv.label}
                      </span>
                      <span className={`text-xs px-2 py-0.5 rounded-full ${fs.bg} ${fs.color}`}>
                        {fs.label}
                      </span>
      ...
      {activeTab === 'compliance' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <h3 className="text-white mb-4">
                {isZh ? '发现严重程度分布' : 'Findings by Severity'}
              </h3>
            ...
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5">
              <h3 className="text-white flex items-center space-x-2 mb-4">
                ...
                <span>AI {isZh ? '合规建议' : 'Compliance Tips'}</span>
              </h3>
              <div className="space-y-3">
                {[
                  {
                    text: isZh
                      ? '采购流程合规评分仅75分，建议加强50万以上采购的系统强制审批控制'
                      : 'Procurement compliance 75, enforce system-level approval for >¥500K',
                    level: 'high',
                  },
                  {
                    text: isZh
                      ? '环保排放已发现2次超标，建议升级除尘系统并增加AI在线监测'
                      : '2 emission violations found, upgrade dust system + add AI monitoring',
                    level: 'high',
                  },
                  {
                    text: isZh
                      ? 'IT权限审计即将启动，AI预扫描发现12个账户存在权限过大风险'
                      : 'IT audit starting, AI pre-scan found 12 over-privileged accounts',
                    level: 'medium',
                  },
                  {
                    text: isZh
                      ? '危废管理可引入电子联单系统，提升合规效率约60%'
                      : 'E-manifest system can improve hazardous waste compliance by 60%',
                    level: 'low',
                  },
                ].map((t, i) => (
                  <div
                    key={i}
                    className={`p-3 rounded-xl border ${t.level === 'high' ? 'bg-red-500/10 border-red-500/15' : t.level === 'medium' ? 'bg-amber-500/10 border-amber-500/15' : 'bg-slate-900/30 border-white/5'}`}
                  >
