...
const SmartSales = React.forwardRef<HTMLDivElement, SmartSalesProps>(({ currentLanguage }, ref) => {
  ...
  return (
    <div ref={ref} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">{isZh ? '智能销售' : 'Smart Sales'}</h2>
          <p className="text-sm text-slate-400 mt-1">
            {isZh
              ? 'AI驱动的销售预测和客户管理'
              : 'AI-powered sales prediction and customer management'}
          </p>
        </div>
          ...
          <div className="bg-slate-800/50 rounded-xl border border-white/5 p-6">
            ...
            <div className="space-y-4">
              {LEADS.map((lead) => (
                <div
                  key={lead.id}
                  className="p-4 rounded-lg bg-slate-900/40 border border-white/5 hover:border-fuchsia-500/30 transition-colors cursor-pointer"
                >
