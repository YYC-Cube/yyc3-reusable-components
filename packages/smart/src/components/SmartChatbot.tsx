...
const DEMO_MESSAGES: ChatMessage[] = [
  ...
  },
  {
    id: '3',
    role: 'user',
    content: '帮我查一下上海汇丰回收的账户余额和最近的订单状态',
    timestamp: '14:33',
  },
  {
    id: '4',
    role: 'bot',
    content:
      '已为您查询上海汇丰回收（客户ID: C-10086）：\n\n💰 账户信息：\n• 信用额度：¥500万\n• 已用额度：¥312万\n• 可用余额：¥188万\n• 应收账款：¥45.6万（逾期7天）\n\n📦 最新订单：\n• ORD-2026031201 废铜100吨 已发货（预计3/15到达）\n• ORD-2026030801 废铝50吨 已签收\n\n⚠️ AI提醒：该客户近30天互动下降60%，建议客户经理主动回访。流失预警风险已提升至45%。',
    timestamp: '14:33',
    confidence: 98,
    source: 'CRM系统+风控系统',
  },
  { id: '5', role: 'user', content: '帮我安排一个回访任务', timestamp: '14:34' },
  {
    ...
    role: 'bot',
    content:
      '已为您创建回访任务：\n\n✅ 任务创建成功\n• 任务：上海汇丰回收客户回访\n• 负责人：王芳（客户经理）\n• 截止日期：2026-03-15\n• 优先级：高\n• 关联流程：客户挽回-WF-008\n\n已同步通知王芳，并将回访提醒添加到她的日历。需要我准备回访话术建议吗？',
    timestamp: '14:34',
    confidence: 94,
    source: '任务系统',
  },
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              {isZh
                ? '全渠道AI客服·知识库·智能工单'
                : 'Omnichannel AI service · Knowledge base · Smart tickets'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: isZh ? '今日会话' : 'Today Sessions',
            value: '164',
            icon: MessageSquare,
            gradient: 'from-indigo-500 to-purple-500',
            sub: isZh ? '2个进行中' : '2 active',
          },
          {
            label: isZh ? 'AI解决率' : 'AI Resolve Rate',
            value: '86.7%',
            icon: Bot,
          ...
          {
            label: isZh ? '平均响应' : 'Avg Response',
            value: '1.2s',
            icon: Zap,
            gradient: 'from-amber-500 to-orange-500',
            sub: isZh ? '人工: 45s' : 'Human: 45s',
          },
          {
            label: isZh ? '满意度' : 'Satisfaction',
            value: '4.7/5',
            icon: Star,
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-lg transition-all ${activeTab === tab.id ? 'bg-gradient-to-r from-indigo-500/20 to-purple-500/20 text-white border border-indigo-500/30' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
            >
              <T className="w-4 h-4" />
      ...
      {activeTab === 'chat' && (
        <div
          className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 flex flex-col"
          style={{ height: '520px' }}
        >
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {DEMO_MESSAGES.map((msg) => (
              ...
              >
                <div
                  className={`max-w-[80%] ${msg.role === 'user' ? 'bg-indigo-500/20 border-indigo-500/30' : 'bg-slate-700/30 border-white/5'} rounded-2xl border p-4`}
                >
                  {msg.role === 'bot' && (
                    <div className="flex items-center space-x-2 mb-2">
                      <Bot className="w-4 h-4 text-indigo-400" />
                      <span className="text-xs text-indigo-400">
                        AI {isZh ? '助手' : 'Assistant'}
                      </span>
                      {msg.confidence && (
                        <span className="text-xs text-gray-500">
                          {isZh ? '置信度' : 'Conf.'} {msg.confidence}%
                        </span>
                      )}
                    </div>
                  )}
                ...
                {SESSIONS.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                      ...
                      <td className="py-3 px-4 text-center">
                        {s.aiHandled ? (
                          <Bot className="w-4 h-4 text-indigo-400 mx-auto" />
                        ) : (
                      ...
                      <td className="py-3 px-4 text-center">
                        {s.satisfaction ? (
                          <span className="flex items-center justify-center space-x-1 text-amber-400">
                            <Star className="w-3 h-3 fill-current" />
                            <span className="text-xs">{s.satisfaction}</span>
                          </span>
                        ) : (
      ...
      {activeTab === 'knowledge' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {KB_ITEMS.map((kb, i) => (
            <div
              key={i}
              className="bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/5 p-5 hover:border-indigo-500/20 transition-all"
            >
              <div className="flex items-center space-x-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {kb.docs.toLocaleString()} {isZh ? '篇文档' : 'docs'}
                  </p>
                </div>
              </div>
              <div className="flex items-center justify-between">
