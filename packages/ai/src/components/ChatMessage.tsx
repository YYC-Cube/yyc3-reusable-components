    ...
    return parts.map((part, index) => {
      if (part.startsWith('```')) {
        ...
        return (
          <div
            key={index}
            className="my-4 border border-green-500/30 bg-black font-mono text-xl relative group/code"
          >
            <div className="flex items-center justify-between px-3 py-1 bg-green-500/10 border-b border-green-500/30">
              ...
              <button
                onClick={handleCopy}
                className="text-green-500/40 hover:text-green-500 transition-colors uppercase text-sm tracking-wider font-bold"
              >
                {copied ? '[COPIED]' : '[COPY]'}
              </button>
            </div>
      ...
      return (
        <p
          key={index}
          className="whitespace-pre-wrap mb-4 last:mb-0 text-green-50/90 leading-relaxed font-mono tracking-wide text-2xl"
        >
          {part}
        </p>
      );
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1">
        ...
        {/* Message Body */}
        <div
          className={`pl-0 ${isUser ? 'text-blue-100' : 'text-green-100'} border-l-2 ${isUser ? 'border-blue-500/30 pl-4' : 'border-green-500/30 pl-4'}`}
        >
          {/* Thinking State */}
          ...
          {!isUser && !isStreaming && (
            <div className="flex items-center gap-4 mt-4 pt-2 border-t border-dashed border-green-500/20 opacity-50 hover:opacity-100 transition-opacity w-fit">
              <button
                onClick={handleCopy}
                className="text-sm text-green-500/50 hover:text-green-500 uppercase tracking-widest flex items-center gap-1"
              >
              ...
              <button className="text-sm text-green-500/50 hover:text-green-500 uppercase tracking-widest flex items-center gap-1">
                <span className="text-green-500">[</span>RTRY
                <span className="text-green-500">]</span>
              </button>
            </div>
          )}
