...
* All public functions and methods must have JSDoc comments (bilingual: Chinese first, then English)
* No `console.log` in production code - use structured logger (`process.stdout.write` for server)
...
* Standard response format: `{ success, data, affectedRows, error, executionTime, timestamp }`
...
* Do not use Tailwind `text-*` size or `font-*` weight classes unless user specifically requests
...
* DELETE operations require WHERE conditions (prevent accidental full-table deletion)
