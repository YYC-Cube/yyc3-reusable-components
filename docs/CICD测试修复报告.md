...
**问题**：effects, business, smart 包缺少 vitest.setup.ts，导致 `@testing-library/jest-dom` 匹配器无法使用
...
**问题**：`3DEffects.tsx`, `MicroInteractions.tsx`, `ParallaxScroll.tsx` 文件缺少默认导出

**解决方案**：
在文件末尾添加默认导出对象，包含所有导出的组件：
